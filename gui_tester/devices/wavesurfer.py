...
from gui_tester.devices.clients.wavesurfer_client import WaveSurferClient
...
class WaveSurfer:
    ...
    def __del__(self):
        ...
        if self.scope is not None:
            self.scope.close()
            self.scope = None
        if self.client.rm is not None:
            self.client.rm.close()
            self.client.rm = None
    ...
    def max_samples(self, n_samples = 0) -> int:
    ...
    def wait_for_sweeps(self, channel, n_sweeps, timeout=100, sleep_interval=0.1) -> (bool, int):
        ...
        if gaaak > sweeps_per_acq:			# check for scope error described above
            ...
            return self.wait_for_sweeps(channel, n_sweeps, timeout, sleep_interval)  # tail recurse to try again
...
if __name__ == '__main__':
    ...
    def screen_dump():
        ...
        with LecroyScope(scope_ip_addr, verbose=False) as scope:
