...
from gui_tester.devices.motor import Motor
...
class ProbeDriveXY:
    def __init__(self, x_ip_addr = None, y_ip_addr = None):
        x_motor_client = MotorClient(x_ip_addr)
        y_motor_client = MotorClient(y_ip_addr)

        self.x_mc = Motor(x_motor_client)
        self.y_mc = Motor(y_motor_client)
